...
if numDims(data0) == gDim % with fixed target set or obstacles
    % New computation
    if keepLast
        data = data0;
    elseif lowMemory
        ...
        data = zeros([data0size(1:gDim) length(tau)]); % generates value fcn set along with time
        data(clns{:}, 1) = data0; % init first timesetp
    end
    
    istart = 2;
elseif numDims(data0) == gDim + 1 % with changing target set or obstacles along time
    ...
    elseif lowMemory
        ...
        data(clns{:}, 1:data0size(end)) = data0; % same dim, but copy corresponding init value fcn into each timestep
    end
...
for i = istart:length(tau)
    ...
    % Reshape value function (maybe from vector to grid's shape)
